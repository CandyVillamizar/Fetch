...
departamentos=[
  ...
  function mostrarTarjetas(items) {
    ...
    items.forEach(item => {
        ...
        card.innerHTML = `
            <div class="card-body" data-category=${item.category}>
              <h5 class="card-title">${item.name}</h5>
              <p class="card-text">${item.description}.</p>
              <div class="d-flex flex-row justify-content-between">
              <button onclick="location.href='detalles.html?id=${item.id}'"class="btn btn-primary">Detalles</button>
              </div>
            </div>`
  ...
  function filtrarTarjetas(){
    ...
    const itemsFiltrados = departamentos.filter(item => 
  ...
  searchInput.addEventListener('input', filtrarTarjetas);
    ...
    for (let i= 0; i<departamentos.length; i++) {
        ...
        function pintarTarjetas(departamentos) {
            let contenedor = document.getElementById("contenedor")
            let tarjeta = document.createElement('div')
            tarjeta.className="card"
            tarjeta.innerHTML = `
            <div class="card-body" data-category=${departamentos[i].category}>
              <h5 class="card-title">${departamentos[i].name}</h5>
              <p class="card-text">${departamentos[i].description}.</p>
              <div class="d-flex flex-row justify-content-between">
              <a href="./detalles.html?id=${departamentos[i].id}" class="btn btn-primary">Detalles</a>
              </div>
            </div>`
            contenedor.appendChild(tarjeta)
          }
          pintarTarjetas(departamentos);
    }

// Función para filtrar departamentos
function filterDepartments() {
  const filteredDepartments = departamentos.filter(department => department.population > 1000000);
  displayDepartments(filteredDepartments);
}

// Función para mostrar departamentos en la lista
function displayDepartments(departments) {
  const departmentList = document.getElementById('contenedor');
  departmentList.innerHTML = ''; // Limpiar la lista antes de agregar nuevos elementos

  departments.forEach(department => {
      const listItem = document.createElement('div');
      listItem.className = 'card';
      listItem.textContent = `${department.name} - Población: ${department.population}`;
      departmentList.appendChild(listItem);
  });
}

// Evento para el botón de filtrar
document.getElementById('filterButton').addEventListener('click', filterDepartments);
