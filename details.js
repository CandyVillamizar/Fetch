...
const departamentos=[
  ...
  let departamento = departamentos.find(e => e.id == idDepartamento)

  document.getElementById("detalle").innerHTML = `
              <h3 class="card-title"> ${departamento.name}</h3>
              <p class="card-text"> ${departamento.description}</p>
              <p>Municipios: ${departamento.municipalities}</p>
              <p>Poblacion: ${departamento.population}</p>
              `

let filteredCities = [];


// Función para obtener ciudades por ID de departamento
async function getCitiesByDepartmentId(departmentId) {
  try {
...
// Función para pintar tarjetas de ciudades
function paintCityCards(departmentId) {
  let container = document.getElementById("contenedor2");
  container.innerHTML = ''; // Limpiar el contenedor antes de pintar nuevas tarjetas

  const citiesForDepartment = filteredCities.filter(city => city.departmentId === departmentId);

  citiesForDepartment.forEach(city => {
    let card = document.createElement('div');
    card.className = "card";
    card.innerHTML = `
      <div class="card-body" data-category="City">
...
// Obtener ciudades para cada departamento en paralelo
async function getCitiesForAllDepartments() {
  const promises = departamentos.map(department => getCitiesByDepartmentId(department.id));
  const allCities = await Promise.all(promises);
  allCities.forEach(cities => filteredCities.push(...cities));
}

// Llamar a la función para obtener las ciudades
getCitiesForAllDepartments().then(() => {
  for (let i= 0; i< departamentos.length; i++) {
    if (idDepartamento==i) {
      getCitiesByDepartmentId(i).then(cities => {
  ...
  // Agregar evento de clic a los elementos de departamento
  const departmentElements = document.querySelectorAll('.department');
  departmentElements.forEach(element => {
    element.addEventListener('click', () => {
      const departmentId = parseInt(element.dataset.id);
      paintCityCards(departmentId);
    });
  });
});

// Manejar el evento del checkbox
const toggleCheckbox2 = document.getElementById('toggle');
toggleCheckbox2.addEventListener('change', function() {
    const naturalAreasContainer = document.getElementById('contenedor2');
    if (this.checked) {
        naturalAreasContainer.style.display = 'Flex'; // Mostrar el contenedor
    } else {
        naturalAreasContainer.style.display = 'none'; // Ocultar el contenedor
    }
    const naturalAreasContainer2 = document.getElementById('titulo-ciudades');
    if (this.checked) {
        naturalAreasContainer2.style.display = 'block'; // Mostrar el contenedor
    } else {
        naturalAreasContainer2.style.display = 'none'; // Ocultar el contenedor
    }
});


if (idDepartamento) {
    const url = `https://api-colombia.com/api/v1/Department/${idDepartamento}/naturalareas`; // Reemplaza con la URL correcta
    ...
    fetch(url)
        ...
        .then(data => {
            const naturalAreasList = document.getElementById('natural-areas');
            naturalAreasList.innerHTML = ''; // Limpiar lista antes de agregar nuevos elementos

            if (data.length > 0) {
                // Usar un Set para rastrear áreas únicas
                const uniqueAreas = new Set();

                data[0].naturalAreas.forEach(area => {
                    // Suponiendo que cada área tiene una propiedad 'name'
                    if (!uniqueAreas.has(area.name)) {
                        uniqueAreas.add(area.name); // Agregar el nombre al Set
                        const listItem = document.createElement('li');
                        listItem.className = 'area';
                        listItem.textContent = area.name; // Agregar el nombre del área
                        naturalAreasList.appendChild(listItem);
                    }
                });
            } else {
                const listItem = document.createElement('li');
                listItem.textContent = 'No se encontraron áreas naturales para este departamento.';
                naturalAreasList.appendChild(listItem);
            }
        })
        .catch(error => {
    ...
    console.error('No se proporcionó un ID de departamento en la URL.');
}

// Manejar el evento del checkbox
const toggleCheckbox = document.getElementById('toggle-areas');
toggleCheckbox.addEventListener('change', function() {
    const naturalAreasContainer = document.getElementById('contenedor3');
    if (this.checked) {
        naturalAreasContainer.style.display = 'block'; // Mostrar el contenedor
    } else {
        naturalAreasContainer.style.display = 'none'; // Ocultar el contenedor
    }
});
